...
while (totalDeCasos > 0) {
    ...
    let numeroDeClientes = parseInt(gets());
    let senhasRecebidas = gets().split(" ");
    let senhasOrdenadas = senhasRecebidas.slice().sort((a,b)=>b-a);
    let mudouDeLugar = 0;

    for (let i = 0; i < numeroDeClientes; i++) {
        if (senhasRecebidas[i] === senhasOrdenadas[i]) {
            mudouDeLugar++;
        }
      }

    console.log(mudouDeLugar);
};